...
class ArbolBinarioBusqueda:
    ...
    def encontrar_abuelo(self, nodo):
        # función que encuentra el abuelo de un nodo dado

        if nodo is not None and nodo != self.raiz:
            # si el nodo no es vacio y no es la raiz, entonces tiene padre y abuelo.
            padre = self.encontrar_padre(nodo)
            abuelo = self.encontrar_padre(padre)
            if abuelo is not None:
                # si el abuelo existe, se retorna.
                return abuelo
        # si el nodo es None, es la raiz o no tiene abuelo, retornamos None.
        return None

    def encontrar_tio(self, nodo):
        # funcion que encuentra el tio de un nodo dado.

        abuelo = self.encontrar_abuelo(nodo)
        if abuelo is not None:
            # si el abuelo existe, entonces el nodo tiene un tio.
            if nodo == abuelo.izquierdo:
                return abuelo.derecho
            else:
                return abuelo.izquierdo
        # si el abuelo no existe, entonces el nodo no tiene tio.
        return None

    def encontrar_padre(self, nodo):
        # verificar si el nodo no es la raiz del arbol.
        if nodo is not None and nodo != self.raiz:
            ...
            # bucle de busqueda.
            while nodo_actual is not None and nodo_actual != nodo:
                
                # actualizar el padre actual y mover el nodo actual al sub-arbol apropiado.
                padre_actual = nodo_actual
            ...
            # comprobar si el nodo buscado fue encontrado.
            if nodo_actual == nodo:
                return padre_actual
        
        # el nodo no tiene padre o no se encontro en el arbol.
        return None
    
    def imprimir_abuelo_y_tio(self, id):
        nodo = self.buscar_nodo(id)
        if nodo is None:
        ...
        tio= None
        if nodo != self.raiz:
            padre = self.encontrar_padre(id)
            abuelo = self.encontrar_padre(padre.id)
            tio = self.encontrar_tio(id)

        if abuelo is None or abuelo.izquierdo is None or abuelo.derecho is None:
            print("el abuelo no tiene dos hijos.")
            return

        if padre is None:
            print("el nodo raiz no tiene abuelo ni tio.")
            return
        ...
        print("el abuelo es:", abuelo.nombre, "con ID:", abuelo.id)
        print("el tío es:", tio.nombre, "con ID:", tio.id)
